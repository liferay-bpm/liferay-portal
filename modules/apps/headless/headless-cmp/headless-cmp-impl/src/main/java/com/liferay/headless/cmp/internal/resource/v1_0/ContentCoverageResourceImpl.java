...
public class ContentCoverageResourceImpl
	...
	private BooleanQuery _createLinkedAssetsQuery(
		...
		booleanQuery.addShouldQueryClauses(
			QueriesUtil.term("cmpProjectObjectEntryIds", projectId));

		String[] relatedCMPTaskObjectEntryIds =
			_getRelatedCMPTaskObjectEntryIds(objectEntry);

		if (relatedCMPTaskObjectEntryIds.length > 0) {
			booleanQuery.addShouldQueryClauses(
				_createTermsQuery(
					"cmpTaskObjectEntryIds", relatedCMPTaskObjectEntryIds));
		}
	...
	private String[] _getRelatedCMPTaskObjectEntryIds(ObjectEntry objectEntry)
		throws Exception {
