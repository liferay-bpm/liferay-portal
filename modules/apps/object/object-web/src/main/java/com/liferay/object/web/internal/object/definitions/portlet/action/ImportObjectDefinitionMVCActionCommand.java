...
public class ImportObjectDefinitionMVCActionCommand
	...
	protected void doProcessAction(
		...
		JSONObject jsonObject = _importObjectDefinition(actionRequest);
	...
	private JSONObject _importObjectDefinition(ActionRequest actionRequest)
		...
		boolean listImport = Validator.isNull(objectDefinitionJSON);
		...
		if (!listImport) {
			return errorsJSONArray.getJSONObject(0);
		}

		return JSONUtil.put(
			"message", errorsJSONArray
		).put(
			"type", "ObjectDefinitionsImportErrorsException"
		);
	}
