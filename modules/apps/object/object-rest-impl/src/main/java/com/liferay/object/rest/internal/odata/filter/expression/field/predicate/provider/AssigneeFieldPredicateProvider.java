...
public class AssigneeFieldPredicateProvider implements FieldPredicateProvider {
	...
	public Predicate getBinaryExpressionPredicate(
		...
		try {
			...
			User user = _userLocalService.getUser(GetterUtil.getLong(right));

			return Predicate.or(
				classNameIdColumn.eq(
					_portal.getClassNameId(Role.class.getName())
				).and(
					classPKColumn.in(_getRoleIds(user))
				).withParentheses(),
				classNameIdColumn.eq(
					_portal.getClassNameId(User.class.getName())
				).and(
					classPKColumn.eq(user.getUserId())
				).withParentheses()
			).withParentheses();
		}
	...
	private Long[] _getRoleIds(User user) throws PortalException {
		UserBag userBag = UserBagFactoryUtil.create(user);

		List<Long> roleIds = ListUtil.concat(
			ListUtil.fromArray(userBag.getRoleIds()),
			TransformUtil.transform(
				_userGroupRoleLocalService.getUserGroupRoles(user.getUserId()),
				UserGroupRoleModel::getRoleId));
