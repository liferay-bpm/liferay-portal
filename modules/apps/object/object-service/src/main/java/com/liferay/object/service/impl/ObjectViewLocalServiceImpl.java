...
public class ObjectViewLocalServiceImpl extends ObjectViewLocalServiceBaseImpl {
	...
	private List<ObjectViewColumn> _addObjectViewColumns(
		...
		return TransformUtil.transform(
			objectViewColumns,
			objectViewColumn -> {
				ObjectViewColumn objectViewColumnNew =
					_objectViewColumnPersistence.create(
						counterLocalService.increment());

				objectViewColumnNew.setCompanyId(user.getCompanyId());
				objectViewColumnNew.setUserId(user.getUserId());
				objectViewColumnNew.setUserName(user.getFullName());
				objectViewColumnNew.setObjectViewId(objectViewId);
				objectViewColumnNew.setObjectFieldName(
					objectViewColumn.getObjectFieldName());
				objectViewColumnNew.setPriority(objectViewColumn.getPriority());

				return _objectViewColumnPersistence.update(objectViewColumnNew);
			});
	}
