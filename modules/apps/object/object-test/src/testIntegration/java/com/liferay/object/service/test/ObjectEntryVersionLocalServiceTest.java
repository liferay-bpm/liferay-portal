...
public class ObjectEntryVersionLocalServiceTest {
	...
	@Test
	public void testAddObjectEntryVersionWithObjectEntryDraftAndScheduleEnabled()
		throws Exception {
		...
		try {
			...
			// Add object entry with display date in the future
			...
			// Update object entry as draft with null display date
			...
			// Update object entry as published with display date in the future
	...
	public void testAddObjectEntryVersionWithObjectEntryScheduleEnabled()
		...
		int originalMaximumVersionsPerEntry =
		...
		try {
			...
			objectEntry = _objectEntryLocalService.updateObjectEntry(
				...
				HashMapBuilder.<String, Serializable>put(
					"displayDate",
					new Date(
						System.currentTimeMillis() + TimeUnit.DAY.toMillis(2))
				).put(
					"textObjectFieldName", "textObjectFieldValue3"
				).build(),
				serviceContext);
			...
			Assert.assertTrue(objectEntry.isApproved());
			Assert.assertEquals(5, objectEntry.getVersion());
			...
			_configurationProvider.saveCompanyConfiguration(
				...
				HashMapDictionaryBuilder.<String, Object>put(
					...
					"maximumVersionsPerEntry", originalMaximumVersionsPerEntry
				).build());
