...
public class BatchEnginePortletDataHandlerTest {
	...
	public void testExportImportListTypeDefinitions() throws Exception {
		...
		JSONArray exportedJSONArray = _getExportedEntriesJSONArray(
			com.liferay.headless.admin.list.type.dto.v1_0.ListTypeDefinition.
				class.getName(),
			larFile, group.getGroupId());

		Assert.assertEquals(2, exportedJSONArray.length());

		_assertExportedListTypeDefinitionAndEntries(
			exportedJSONArray, listTypeDefinition1, listTypeEntry1,
			listTypeEntry2);

		_assertExportedListTypeDefinitionAndEntries(
			exportedJSONArray, listTypeDefinition2, listTypeEntry3);
		...
		_assertImportedListTypeDefinitionAndEntries(
			listTypeDefinition1, listTypeEntry1, listTypeEntry2);

		_assertImportedListTypeDefinitionAndEntries(
			listTypeDefinition2, listTypeEntry3);
	...
	private void _assertExportedListTypeDefinitionAndEntries(
		JSONArray exportedListTypeDefinitionsJSONArray,
		ListTypeDefinition expectedListTypeDefinition,
		...
		JSONObject exportedListTypeDefinitionJSONObject =
			_getExportedListTypeDefinitionJSONObject(
				exportedListTypeDefinitionsJSONArray,
				expectedListTypeDefinition);

		Assert.assertEquals(
			expectedListTypeDefinition.getExternalReferenceCode(),
			exportedListTypeDefinitionJSONObject.getString(
				"externalReferenceCode"));

		Assert.assertEquals(
			expectedListTypeDefinition.getName(LocaleUtil.US),
			exportedListTypeDefinitionJSONObject.getString("name"));
		...
		Assert.assertEquals(
			expectedListTypeEntries.length, listTypeEntriesJSONArray.length());

		Set<String> exportedERCs = new HashSet<>();

		for (int i = 0; i < listTypeEntriesJSONArray.length(); i++) {
			exportedERCs.add(
				listTypeEntriesJSONArray.getJSONObject(
					i
				).getString(
					"externalReferenceCode"
				));
		...
		for (ListTypeEntry expected : expectedListTypeEntries) {
			Assert.assertTrue(
				exportedERCs.contains(expected.getExternalReferenceCode()));
		}
	}

	private void _assertImportedListTypeDefinitionAndEntries(
			ListTypeDefinition expectedlistTypeDefinition,
			ListTypeEntry... expectedListTypeEntries)
		throws Exception {
		...
		List<ListTypeEntry> listTypeEntries =
			_listTypeEntryLocalService.getListTypeEntries(
				actualListTypeDefinition.getListTypeDefinitionId());

		Set<String> importedERCs = new HashSet<>();

		for (ListTypeEntry entry : listTypeEntries) {
			importedERCs.add(entry.getExternalReferenceCode());
		}

		for (ListTypeEntry expected : expectedListTypeEntries) {
			Assert.assertTrue(
				importedERCs.contains(expected.getExternalReferenceCode()));
		}
	}
	...
	private JSONArray _getExportedEntriesJSONArray(
	...
	private void _testExportImportObjectEntriesWithRelatedObjectEntries(
		...
		JSONArray exportedObjectEntriesJSONArray = _getExportedEntriesJSONArray(
		...
		if (Objects.equals(
			...
			exportedObjectEntriesJSONArray = _getExportedEntriesJSONArray(
