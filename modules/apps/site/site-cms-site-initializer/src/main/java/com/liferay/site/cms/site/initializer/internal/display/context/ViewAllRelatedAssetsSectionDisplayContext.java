...
public class ViewAllRelatedAssetsSectionDisplayContext
	...
	protected String getLinkedAssetsFilterString() {
		String projectFilterString = getLinkedAssetsFilterString(
			"cmpProjectIds", objectEntry.getObjectEntryId());

		String taskFilterString = getLinkedAssetsFilterString(
			"cmpTaskIds", _getRelatedTaskObjectEntryIds());

		if (Validator.isNull(taskFilterString)) {
			return projectFilterString;
		}

		return StringBundler.concat(
			"(", projectFilterString, " or ", taskFilterString, ")");
	}

	private long[] _getRelatedTaskObjectEntryIds() {
		try {
