...
public abstract class BaseRelatedAssetsSectionDisplayContext
	...
	protected String getCMSSectionFilterString() {
		String linkedFilterString = getLinkedFilterString();

		if (Validator.isNull(linkedFilterString)) {

			// No linked asset can have a negative object entry ID, so this
			// matches nothing.

			linkedFilterString = "cmpTaskIds/any(x:x eq -1)";
		}

		return appendStatus(
			StringBundler.concat(
				"(cmsSection eq 'contents' or cmsSection eq 'files') and ",
				linkedFilterString, " and rootDescendantNode eq false"));
	}

	protected abstract String getLinkedFilterString();
	...
	protected String getLinkedObjectEntryIdsFieldName(
		ObjectDefinition objectDefinition) {
