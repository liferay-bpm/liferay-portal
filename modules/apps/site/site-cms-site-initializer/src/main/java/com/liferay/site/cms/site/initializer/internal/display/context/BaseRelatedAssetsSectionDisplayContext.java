...
public abstract class BaseRelatedAssetsSectionDisplayContext
	...
	protected String getCMSSectionFilterString() {
		String linkedAssetsFilterString = getLinkedAssetsFilterString();

		if (Validator.isNull(linkedAssetsFilterString)) {
			linkedAssetsFilterString = "cmpTaskObjectEntryIds/any(x:x eq -1)";
		}

		return appendStatus(
			StringBundler.concat(
				"(cmsSection eq 'contents' or cmsSection eq 'files') and ",
				linkedAssetsFilterString, " and rootDescendantNode eq false"));
	}

	protected abstract String getLinkedAssetsFilterString();
