...
public class ListTypeDefinitionUserIdUpgradeProcessTest {
	...
	public void testUpgrade() throws Exception {
		...
		Assert.assertEquals(userId, _listTypeDefinition.getUserId());

		UpgradeProcess upgradeProcess = UpgradeTestUtil.getUpgradeStep(
			_upgradeStepRegistrator,
			"com.liferay.list.type.internal.upgrade.v1_3_2." +
				"ListTypeDefinitionUserIdUpgradeProcess");
		...
		_listTypeDefinition =
			_listTypeDefinitionLocalService.
				getListTypeDefinitionByUuidAndCompanyId(
					_listTypeDefinition.getUuid(),
					TestPropsValues.getCompanyId());

		Assert.assertEquals(
			defaultServiceAccountUser.getUserId(),
			_listTypeDefinition.getUserId());
	}
